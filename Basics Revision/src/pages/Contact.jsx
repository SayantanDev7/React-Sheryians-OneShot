...
const Contact = () => {
  ...
  const outlet = useOutlet() // Dynamically detects if a nested child route (like profile or settings) is active
  ...
  return (
    <div className="min-h-[calc(100vh-68px)] bg-gray-950 text-gray-100 py-12 px-4 sm:px-6 lg:px-8 flex flex-col items-center justify-start gap-8">
      {/* 
        PREMIUM SUB-NAVIGATION TAB SWITCHER:
        Allows users to seamlessly transition between the Contact Form, Profile, and Settings nested subviews.
      */}
      <div className="flex bg-gray-900 border border-gray-800 p-1.5 rounded-xl shadow-xl z-10 w-full max-w-md">
        <NavLink 
          to="/contact" 
          end
          className={({ isActive }) => 
            `grow text-center px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 cursor-pointer ${
              isActive 
      ...
      {/* Main card container. The size is locked to max-w-md for perfect visual consistency. */}
      <div className="max-w-md w-full bg-gray-900 border border-gray-800 rounded-2xl p-8 shadow-xl transition-all duration-300">
        {outlet ? (
          /* Render active sub-route page dynamically when user switches tabs */
          <Outlet />
        ) : (
          /* Default state: Render Contact Form if on the index /contact route */
          <div>
